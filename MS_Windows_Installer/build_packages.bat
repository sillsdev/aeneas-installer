...
set PATH=%PATH%;%PF32%\FFmpeg\bin;%PF32%\eSpeak NG;%PF32%\Python38;%PF32%\Python38\Scripts;%PF32%\Git\usr\bin;%PF32%\Git\mingw32\bin
...
IF NOT EXIST "%cd%\espeak-ng-1.50-win32.exe" (
  echo Downloading eSpeak-ng...
  %CURL% https://github.com/espeak-ng/espeak-ng/releases/download/1.50/espeak-ng-20191129-b702b03-x86.msi -o %cd%\espeak-ng-1.50-x86.msi
  ...
  lessmsi-v1.6.91\lessmsi.exe x espeak-ng-1.50-x86.msi
  "%INNOPATH%\ISCC.exe" eSpeak-ng_Installer.iss
)
IF EXIST "%cd%\espeak-ng-1.50-win32.exe" (
   echo Installing eSpeak-ng...
   "%cd%\espeak-ng-1.50-win32.exe" /SILENT /ALLUSERS
...
REM DEL "%cd%\ffmpeg-4.3-win32-static.zip"
IF NOT EXIST "%cd%\ffmpeg-4.3-win32-static.zip" (
  echo Downloading FFmpeg...
  %CURL% https://ffmpeg.zeranoe.com/builds/win32/static/ffmpeg-4.3-win32-static.zip -o %cd%\ffmpeg-4.3-win32-static.zip
)
IF NOT EXIST "%cd%\ffmpeg-4.3-win32-static.exe" (
  "%SEVENZ%" x ffmpeg-4.3-win32-static.zip -aoa
  rmdir /q/s ffmpeg-4.3
  move /y ffmpeg-4.3-win32-static ffmpeg-4.3
