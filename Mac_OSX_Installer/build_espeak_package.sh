...
if [ ! -f "aeneas-mac-installer-packages/espeak-ng-$ESPEAK_VER.pkg" ]; then
	...
	sudo port -Npfu uninstall espeak-ng pcaudiolib sonic
	sudo port -Npfk install espeak-ng
	...
	cd "$CURDIR"
	pkgbuild --prior `port work espeak-ng`/espeak-ng-$ESPEAK_VER-component.pkg --version "$ESPEAK_VER" --root "$BUILDTMP/Payload" --scripts "$BUILDTMP/Scripts" "espeak-ng-$ESPEAK_VER.pkg"
        [ $? = 0 ] || exit 1
